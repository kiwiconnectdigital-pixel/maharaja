...
const removeId = (ids, id) => {
  ...
  return normalizedIds.filter((existingId) => existingId !== numericId);
};

const getOrCreateFirm = async (firmName, firmAddress = null) => {
  if (!firmName) {
    throw createError.BadRequest("Firm name is required");
  }
  ...
  if (!firm) {
    ...
    if (!firm.firmAddress && cleanValue(firmAddress)) {
      firm.firmAddress = cleanValue(firmAddress);
...
module.exports = {
  register: async (req, res, next) => {
    try {
      const { name, email, dob, image, bloodGroup, mobile, firms, role } =
        req.body;
      ...
      if (!mobile) {
        throw createError.BadRequest("Mobile number is required");
      }

      if (!firms || !Array.isArray(firms) || firms.length === 0) {
        throw createError.BadRequest("At least one firm is required");
      }
      ...
      */
      const user = await User.create({
        ...
        email: cleanValue(email) ? cleanValue(email).toLowerCase() : null,
      ...
      */
      ...
      for (const firmData of firms) {
        ...
        */
        ...
        |--------------------------------------------------------------------------
        */
        user.firm_ids = addUniqueId(user.firm_ids, firm.id);
        ...
        |--------------------------------------------------------------------------
        */
        firm.user_ids = addUniqueId(firm.user_ids, user.id);
  ...
  login: async (req, res, next) => {
    try {
      ...
      if (!mobile || !password) {
        throw createError.BadRequest("Mobile number and password are required");
      }
      ...
      */
      ...
      |--------------------------------------------------------------------------
      */
      const firmIds = normalizeIds(loggedInUser.firm_ids);
      ...
      if (firmIds.length > 0) {
        firms = await Firm.findAll({
          ...
          attributes: ["id", "firmName", "firmAddress"],
        });
      }
      ...
      |--------------------------------------------------------------------------
      */
      const accessToken = await signAccessToken(loggedInUser.id);

      const refreshToken = await signRefreshToken(loggedInUser.id);
  ...
  refreshToken: async (req, res, next) => {
    try {
      ...
      if (!refreshToken) {
        throw createError.BadRequest("Refresh token required");
      }
      ...
      if (user.is_inactive) {
        throw createError.Unauthorized("User account is inactive");
      }
  ...
  getAllUsers: async (req, res, next) => {
    try {
      ...
      */
      ...
      users.forEach((user) => {
        const ids = normalizeIds(user.firm_ids);
      ...
      firms.forEach((firm) => {
        firmMap[firm.id] = firm.toJSON();
      });

      const finalUsers = users.map((user) => {
        const json = user.toJSON();

        const firmIds = normalizeIds(json.firm_ids);
        ...
        return {
          ...
          firms: firmIds.map((id) => firmMap[id]).filter(Boolean),
        };
      });
  ...
  getFirm: async (req, res, next) => {
    try {
      ...
      |--------------------------------------------------------------------------
      */
      const userIds = normalizeIds(firm.user_ids);
  ...
  try {
    ...
    if (firmName !== undefined) {
      ...
      firm.firmName = firmName.trim();
    }

    if (firmAddress !== undefined) {
    ...
    if (user_ids !== undefined) {
      ...
      if (newUserIds.length > 0) {
        ...
        if (users.length !== newUserIds.length) {
          const foundIds = users.map(
            (user) => Number(user.id)
          );

          const missingIds = newUserIds.filter(
      ...
      // ----------------------------------------------------------
      // Old users connected to this firm
      // ----------------------------------------------------------

      const oldUserIds = normalizeIds(
        firm.user_ids
      );

      // ----------------------------------------------------------
      // Remove this firm from users who are no longer connected
      // ----------------------------------------------------------

      const removedUserIds = oldUserIds.filter(
        (userId) => !newUserIds.includes(
          Number(userId)
        )
      );
      ...
      for (const userId of removedUserIds) {
        ...
        if (user) {
          const currentFirmIds =
            normalizeIds(user.firm_ids);

          user.firm_ids =
            currentFirmIds.filter(
              (firmId) =>
                Number(firmId) !== Number(id)
            );
  ...
  getAllFirms: async (req, res, next) => {
    ...
    */
    ...
    |--------------------------------------------------------------------------
    */
      const { rows: firms, count: totalFirms } = await Firm.findAndCountAll({
        order: [["id", "DESC"]],
        limit,
        offset,
      });
    ...
    */
      const finalFirms = firms.map((firm) => {
        ...
        const userIds = normalizeIds(json.user_ids);
        ...
        return {
          ...
          users: userIds.map((id) => userMap[id]).filter(Boolean),
        };
      });
  ...
  addFirmToUser: async (req, res, next) => {
    try {
      ...
      |--------------------------------------------------------------------------
      */
      const firm = await getOrCreateFirm(firmName, firmAddress);
  ...
  removeFirmFromUser: async (req, res, next) => {
    try {
      ...
      |--------------------------------------------------------------------------
      */
      user.firm_ids = removeId(user.firm_ids, firmId);
      ...
      |--------------------------------------------------------------------------
      */
      firm.user_ids = removeId(firm.user_ids, userId);
  ...
  bulkUpload: async (req, res, next) => {
    try {
      if (!req.file) {
        throw createError.BadRequest("Excel file is required");
      }
      ...
      */
      ...
      if (!rows.length) {
        throw createError.BadRequest("Excel file is empty");
      }
      ...
      */
      for (let index = 0; index < rows.length; index++) {
        ...
        try {
          ...
          const bloodGroup = cleanValue(row.bloodGroup);

          const mobile = cleanValue(row.mobile);

          const firmName = cleanValue(row.firmName);

          const firmAddress = cleanValue(row.firmAddress);
          ...
          */
          ...
          if (!firm) {
            ...
            |--------------------------------------------------------------------------
            */
            if (!firm.firmAddress && firmAddress) {
              firm.firmAddress = firmAddress;
          ...
          |--------------------------------------------------------------------------
          */
          const oldFirmIds = normalizeIds(user.firm_ids);

          const newFirmIds = addUniqueId(oldFirmIds, firm.id);

          if (JSON.stringify(oldFirmIds) !== JSON.stringify(newFirmIds)) {
            user.firm_ids = newFirmIds;
          ...
          |--------------------------------------------------------------------------
          */
          const oldUserIds = normalizeIds(firm.user_ids);

          const newUserIds = addUniqueId(oldUserIds, user.id);
  ...
  search: async (req, res, next) => {
    try {
      const { q = "", firm_id, page = 1, limit = 10 } = req.query;

      const searchText = q.trim();

      if (!searchText) {
        throw createError.BadRequest("Search keyword is required");
      }
      ...
      const [firmResult, productResult] = await Promise.all([
      ...
      res.json({
        ...
        firms: {
          ...
          pagination: {
            currentPage,
            limit: perPage,
            totalPages: Math.ceil(firmResult.count / perPage),
            hasNextPage: currentPage < Math.ceil(firmResult.count / perPage),
        ...
        products: {
          ...
          pagination: {
            currentPage,
            limit: perPage,
            totalPages: Math.ceil(productResult.count / perPage),
            hasNextPage: currentPage < Math.ceil(productResult.count / perPage),
            hasPreviousPage: currentPage > 1,
          },
        },
      });
